...
test(`01 : Create Accounts`,async ({page,request}) => {
    page = await navigateToItem("Sales","Accounts");
    await page.locator("//a[@title='New']").click({timeout:5000});
    await page.locator("//label[text()='Account Name']/..//input").fill(acc_details.create_Account.acc_Name);
